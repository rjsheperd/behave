...
/*! \brief SIGContain constructor.
    ...
    \param[in] flank      Apply SIGContainResources assigned to the Left or Right
                          flank.  SIGContainResources assigned to Both flanks have
                          half their production rate applied to this flank.
    \param[in] force      Pointer to the SIGContainForce applied to the fire.
    \param[in] attackTime Elapsed time since fire report that SIGContainForces
...
SIGContain::SIGContain(
    ...
    m_diurnalSpreadRate(diurnalROS)
{
...
//------------------------------------------------------------------------------
/*! \brief SIGContain class destructor.
 */

SIGContain::~SIGContain( void )
{
  delete m_force;
}
...
/*! \brief Determines du/dh for a particular u, h, and p,
    ...
    which is indicative of the SIGContainResources being overrun by the fire,
    ...
    \retval TRUE if SIGContainResources are not overrun and d has valid result.
    \retval FALSE if SIGContainResources are overrun, and m_status is
            set to Overrun.
 */

bool SIGContain::calcUh( double p, double h, double u, double *d )
{
...
void SIGContain::calcCoordinates( void )
{
...
void SIGContain::containLog( bool dolog, const char *fmt, ... ) const
{
    if ( dolog )
    {
...
double SIGContain::containPsi( double u, double eps2 )
{
...
//------------------------------------------------------------------------------
/*! \brief API access to the time when all the SIGContainResources are exhausted.

    \return Time when all the SIGContainResources are exhausted
            (minutes since report).
 */

double SIGContain::exhaustedTime( void ) const
{
    return( m_exhausted );
}
...
double SIGContain::attackDistance( void ) const
{
    return( m_attackDist );
}
...
double SIGContain::attackPointX( void ) const
{
    return( m_x );
}
...
double SIGContain::attackPointY( void ) const
{
    return( m_y );
}
...
double SIGContain::attackTime( void ) const
{
    return( m_attackTime );
}
...
double SIGContain::distanceStep( void ) const
{
    return( m_distStep );
}
...
double SIGContain::fireBackAtAttack( void ) const
{
    return( m_attackBack );
}
...
double SIGContain::fireBackAtReport( void ) const
{
    return( m_reportBack );
}
...
double SIGContain::fireHeadAtAttack( void ) const
{
    return( m_attackHead );
}
...
double SIGContain::fireHeadAtReport( void ) const
{
    return( m_reportHead );
}
...
double SIGContain::fireLwRatioAtReport( void ) const
{
    return( m_lwRatio );
}
...
double SIGContain::fireReportTime( void ) const
{
    return( m_reportTime );
}
...
double SIGContain::fireSizeAtReport( void ) const
{
    return( m_reportSize );
}
...
double SIGContain::fireSpreadRateAtBack( void ) const
{
    return( m_reportBack );
}
...
double SIGContain::fireSpreadRateAtReport( void ) const
{
    return( m_reportRate );
}

//------------------------------------------------------------------------------
/*! \brief Access to the SIGContainForce assigned to the fire.

    \return Pointer to the SIGContainForce assigned to the fire.
 */

SIGContainForce* SIGContain::force( void ) const
{
    return( m_force );
}
...
double SIGContain::headPosition( double minutesSinceReport ) const
{
...
double SIGContain::productionRate( double fireHeadPosition ) const
{
    //----------------------------------------------
    //    MAF 6/2010
...
double SIGContain::productionRatio( double fireHeadPosition )
{
    //--------------------------------------------------------
    //    MAF 6/2010
...
double SIGContain::setFireStartTimeMinutes(int starttime)
{
...
double SIGContain::getDiurnalSpreadRate(double minutesSinceReport) const
{
     //long i, loc;
     long CurrentHour;
     double CurrentTime;
     ...
     return m_diurnalSpreadRate->at(CurrentHour);
}
...
//------------------------------------------------------------------------------
/*! \brief Access to the number of SIGContainResources in the containment force.

    \return Number of SIGContainResources in the SIGContainForce.
 */

int SIGContain::resources( void ) const
{
...
/*! \brief Access to the arrival time of the specified ContainmentResouce.
    ...
    \param[in] index Index (base 0) of the SIGContainResource.  Indices are
    assigned in the order that the SIGContainResources are added to the
    SIGContainForce.

    \return SIGContainResource's arrival time (minutes since fire was
    reported).
 */

double SIGContain::resourceArrival( int index ) const
{
...
/*! \brief Access to the base cost of the specified ContainmentResouce.
    ...
    \return SIGContainResource's base cost.
 */

double SIGContain::resourceBaseCost( int index ) const
{
...
/*! \brief Access to the specified ContainmentResouce's description.
    ...
    \return SIGContainResource's description.
 */

const char * SIGContain::resourceDescription( int index ) const
{
...
/*! \brief Access to the duration time of the specified ContainmentResouce.
    ...
    \return SIGContainResource's duration time (minutes).
 */

double SIGContain::resourceDuration( int index ) const
{
...
/*! \brief Access to the hourly cost of the specified ContainmentResouce.
    ...
    \return SIGContainResource's hourly cost.
 */

double SIGContain::resourceHourCost( int index ) const
{
...
/*! \brief Access to the holdable fireline production rate of the
    ...
    \return SIGContainResource's holdable fireline production rate (ch/h).
 */

double SIGContain::resourceProduction( int index ) const
{
...
void SIGContain::setAttack( ContainFlank flank, SIGContainForce *force,
...
void SIGContain::setReport( double reportSize, double reportRate, double lwRatio,
        double distStep )
{
    m_reportSize = reportSize;
    m_reportRate = reportRate;
    ...
    //--------------------------------------------------------
    for(long i=0; i<24; i++)
          m_diurnalSpreadRate->swap(i, reportRate);
    //--------------------------------------------------------
...
int SIGContain::simulationStep( void ) const
{
    return( m_step );
}
...
double SIGContain::simulationTime( void ) const
{
    return( m_time );
}
...
/*! \brief Access to the containment simulation status.
    ...
    \return Attack tactic ContainStatus
...
ContainStatus SIGContain::status( void ) const
{
    return( m_status );
}
...
double SIGContain::spreadRate( double /* minutesSinceReport */ ) const
{
    return( m_reportRate );
}
...
ContainStatus SIGContain::step( void )
{
...
/*! \brief Access to the attack tactic.
    ...
    \return Attack tactic ContainTactic
        - HeadAttack = 0
        - RearAttack = 1
 */

ContainTactic SIGContain::tactic( void ) const
{
    return( m_tactic );
}
...
double SIGContain::timeSinceReport( double headPos ) const
{
...
const char * SIGContain::printStatus(ContainStatus cs) {
    const char * status;
    switch (cs) {
        case Unreported:
